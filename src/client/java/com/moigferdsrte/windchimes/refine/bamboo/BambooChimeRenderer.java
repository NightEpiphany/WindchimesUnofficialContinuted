...
public class BambooChimeRenderer implements BlockEntityRenderer<BambooChimeBlockEntity, ChimeBlockEntityRenderState> {

    private final ChimeModel root;

    public BambooChimeRenderer(BlockEntityRendererFactory.Context ctx) {
        this.root = new ChimeModel(ctx.getLayerModelPart(RefineClient.CHIME));
    }

    @Override
    public void updateRenderState(BambooChimeBlockEntity entity, ChimeBlockEntityRenderState state, float tickDelta, Vec3d cameraPos, @Nullable ModelCommandRenderer.CrumblingOverlayCommand crumblingOverlay) {
    ...
    public void render(ChimeBlockEntityRenderState state, MatrixStack matrices, OrderedRenderCommandQueue queue, CameraRenderState cameraState) {
        ...
        RenderLayer renderLayer = RenderLayers.entityCutoutNoCull(Identifier.of(Refine.MOD_ID, "textures/entity/chime/bamboo_chime.png"));
