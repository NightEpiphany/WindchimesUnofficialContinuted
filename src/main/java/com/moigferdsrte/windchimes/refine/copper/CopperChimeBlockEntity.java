...
public class CopperChimeBlockEntity extends BlockEntity {
    ...
    public CopperChimeBlockEntity(BlockPos pos, BlockState state) {
        super(BlockEntityReg.COPPER, pos, state);
        this.ringingTicks = 0;
    ...
    public boolean onSyncedBlockEvent(int type, int data) {
        if(type == 1) {
            if (data == 0) {
                ...
                assert this.world != null;
                this.world.playSound(null, this.pos, SoundReg.COPPER_QUIET, SoundCategory.RECORDS,
                ...
                assert this.world != null;
                this.world.playSound(null, this.pos, SoundReg.COPPER_LOUD, SoundCategory.RECORDS,
                        0.9f + this.world.random.nextFloat() * 0.2f,
                        0.8f + this.world.random.nextFloat() * 0.4f);
            }
            return true;
    ...
    public static void clientTick(World world, BlockPos pos, BlockState state, CopperChimeBlockEntity blockEntity) {
        tick(world, pos, state, blockEntity);
    }

    public static void serverTick(World world, BlockPos pos, BlockState state, CopperChimeBlockEntity blockEntity) {
        tick(world, pos, state, blockEntity);
    }

    public static void tick(World world, BlockPos pos, BlockState blockState, CopperChimeBlockEntity that) {
        if(world.isClient()) {
