...
public class BambooChimeBlock extends BlockWithEntity {

    public static final MapCodec<BambooChimeBlock> CODEC = createCodec(BambooChimeBlock::new);

    private static final VoxelShape SHAPE = Block.createCuboidShape(4.0, 8.0, 4.0, 12.0, 16.0, 12.0);

    public BambooChimeBlock(Settings settings) {
        super(settings);
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, BlockEntityReg.BAMBOO, world.isClient() ? BambooChimeBlockEntity::clientTick : BambooChimeBlockEntity::serverTick);
    }
    ...
    protected void onProjectileHit(World world, BlockState state, BlockHitResult hit, ProjectileEntity projectile) {
        BambooChimeBlockEntity here = ((BambooChimeBlockEntity)world.getBlockEntity(hit.getBlockPos()));
        if (here == null) return;
        here.ring(true);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        BambooChimeBlockEntity here = ((BambooChimeBlockEntity)world.getBlockEntity(pos));
        assert here != null;
    ...
    @Override
    protected MapCodec<BambooChimeBlock> getCodec() {
        return CODEC;
    }
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new BambooChimeBlockEntity(pos, state);
    }
}
