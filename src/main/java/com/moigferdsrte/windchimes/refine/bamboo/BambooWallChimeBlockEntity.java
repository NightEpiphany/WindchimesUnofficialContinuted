...
public class BambooWallChimeBlockEntity extends BlockEntity {
    ...
    public BambooWallChimeBlockEntity(BlockPos pos, BlockState state) {
        super(BlockEntityReg.WALL_BAMBOO, pos, state);
        this.ringingTicks = 0;
    ...
    public boolean onSyncedBlockEvent(int type, int data) {
        if(type == 1) {
            if (data == 0) {
                ...
                assert this.world != null;
                this.world.playSound(null, this.pos, SoundReg.BAMBOO_QUIET, SoundCategory.RECORDS,
                ...
                assert this.world != null;
                this.world.playSound(null, this.pos, SoundReg.BAMBOO_LOUD, SoundCategory.RECORDS,
                        0.9f + this.world.random.nextFloat() * 0.2f,
                        0.8f + this.world.random.nextFloat() * 0.4f);
            }
            return true;
    ...
    public static void clientTick(World world, BlockPos pos, BlockState state, BambooWallChimeBlockEntity blockEntity) {
        tick(world, pos, state, blockEntity);
    }

    public static void serverTick(World world, BlockPos pos, BlockState state, BambooWallChimeBlockEntity blockEntity) {
        tick(world, pos, state, blockEntity);
    }

    public static void tick(World world, BlockPos pos, BlockState blockState, BambooWallChimeBlockEntity that) {
        if(world.isClient()) {
