...
public class IronChimeBlockEntity extends BlockEntity {
    ...
    public IronChimeBlockEntity(BlockPos pos, BlockState state) {
        super(BlockEntityReg.IRON, pos, state);
        this.ringingTicks = 0;
    ...
    public boolean onSyncedBlockEvent(int type, int data) {
        if(type == 1) {
            if (data == 0) {
                ...
                this.world.playSound(null, this.pos, SoundReg.IRON_QUIET, SoundCategory.RECORDS,
                ...
                this.world.playSound(null, this.pos, SoundReg.IRON_LOUD, SoundCategory.RECORDS,
                        0.9f + this.world.random.nextFloat() * 0.2f,
                        0.8f + this.world.random.nextFloat() * 0.4f);
            }
            return true;
    ...
    public static void clientTick(World world, BlockPos pos, BlockState state, IronChimeBlockEntity blockEntity) {
        tick(world, pos, state, blockEntity);
    }

    public static void serverTick(World world, BlockPos pos, BlockState state, IronChimeBlockEntity blockEntity) {
        tick(world, pos, state, blockEntity);
    }

    public static void tick(World world, BlockPos pos, BlockState blockState, IronChimeBlockEntity that) {
        if(world.isClient()) {
