...
public class IronChimeBlock extends BlockWithEntity {

    public static final MapCodec<IronChimeBlock> CODEC = createCodec(IronChimeBlock::new);

    private static final VoxelShape SHAPE = Block.createCuboidShape(4.0, 8.0, 4.0, 12.0, 16.0, 12.0);

    public IronChimeBlock(Settings settings) {
        super(settings);
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, BlockEntityReg.IRON, world.isClient() ? IronChimeBlockEntity::clientTick : IronChimeBlockEntity::serverTick);
    }
    ...
    protected void onProjectileHit(World world, BlockState state, BlockHitResult hit, ProjectileEntity projectile) {
        IronChimeBlockEntity here = ((IronChimeBlockEntity)world.getBlockEntity(hit.getBlockPos()));
        if (here == null) return;
        here.ring(true);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        IronChimeBlockEntity here = ((IronChimeBlockEntity)world.getBlockEntity(pos));
        assert here != null;
    ...
    @Override
    protected MapCodec<IronChimeBlock> getCodec() {
        return CODEC;
    }
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new IronChimeBlockEntity(pos, state);
    }
}
